...
MergeDiff.prototype.merge = function (delta, path, context, options) {
    ...
    this._mergeObject(this.object, utils.wrapByPath(delta, path), "", context, options, 0);
};

MergeDiff.prototype.override = function (delta, path, context, options) {
    ...
    this._mergeObject(this.object, utils.wrapByPath(delta, path), "", context, options, 0, path);
};

MergeDiff.prototype.deleteAtPath = function (path) {
...
MergeDiff.prototype._mergeObject = function (source, delta, path, context, options, level, override) {
    var keys,
        key,
        i,
        length,
    ...
    for (i = 0; i < length; i++) {
        ...
        if (utils.isValid(delta[key]) && delta[key] !== source[key]) {
            levelDiffs = this._mergeAtKey(source, delta, path, key, levelDiffs, context, options, isSourceArray, override);
        }

    }

    // find keys at this level that does not exists at the source object
    // delete them
    if (override) {
...
MergeDiff.prototype._mergeAtKey = function (source, delta, path, key, levelDiffs, context, options, isSourceArray, override) {
    ...
    if (source[srcKey] === undefined) {
        if (options.mergeAdditions && delta[key] !== options.deleteKeyword) {
            ...
            if (isObjectMerge) {
                ...
                childDiffs = this._mergeObject(source[srcKey], mergingValue, utils.concatPath(path, key), context, options, _.isArray(source[srcKey]), override);
    ...
    // existing
        ...
        // update
            ...
            if (isObjectMerge) {
                ...
                childDiffs = this._mergeObject(source[srcKey], mergingValue, utils.concatPath(path, key), context, options, isExistingValueArray, override);
                levelDiffs.addChildTracking(childDiffs, key);
